...
| GET | `/api/showcase/projects/` | одобренные заявки (`approved`) | одобренные заявки своего института |
...
#### Поведение по ролям
- Возвращаются только заявки в финальном статусе **`approved`** (одобрена).
- **admin / cpds** — все одобренные заявки; фильтр по институту не применяется.
- **institute_validator** — одобренные заявки своего института (по причастным подразделениям или целевым институтам).
