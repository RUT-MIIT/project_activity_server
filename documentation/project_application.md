...
# раздел «**Нумерация заявок»**

| название | описание | Тип | обязательно | Пояснения |
| --- | --- | --- | --- | --- |
| application_year | Год заявки | целое число | нет | Автоматически устанавливается при создании (год создания заявки) |
| year_sequence_number | Номер заявки внутри года | целое число | нет | Автоматически генерируется при создании, начинается с 1 для каждого года |
| print_number | Номер для печати | строка | нет | Автоматически генерируется в формате YY-00001 (последние 2 цифры года + тире + номер с ведущими нулями, 5 цифр) |

**Примечания:**
- Номера генерируются автоматически при создании заявки
- Нумерация начинается с 1 для каждого года
- При смене года нумерация сбрасывается и начинается заново с 1
- Номер учитывает максимальный номер заявки в году, а не общее количество (учитываются пропуски)
- Пара (application_year, year_sequence_number) уникальна